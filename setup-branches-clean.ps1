# Script alternativo usando Git Subtree para branches más limpios
...
$carpetas = @("APLICACIÓN MÓVIL", "DISPOSITIVOS", "MATERIALES", "SERVIDOR")
...
git rm -rf . --quiet
if (Test-Path "APLICACIÓN MÓVIL") {
    Get-ChildItem "APLICACIÓN MÓVIL" -Recurse | ForEach-Object {
        $destPath = $_.FullName.Replace((Get-Location).Path + "\APLICACIÓN MÓVIL", (Get-Location).Path)
        if ($_.PSIsContainer) {
...
git rm -rf . --quiet
if (Test-Path "DISPOSITIVOS") {
    Get-ChildItem "DISPOSITIVOS" -Recurse | ForEach-Object {
        $destPath = $_.FullName.Replace((Get-Location).Path + "\DISPOSITIVOS", (Get-Location).Path)
        if ($_.PSIsContainer) {
            New-Item -ItemType Directory -Path $destPath -Force | Out-Null
        } else {
            Copy-Item $_.FullName -Destination $destPath -Force
...
git rm -rf . --quiet
if (Test-Path "MATERIALES") {
    Get-ChildItem "MATERIALES" -Recurse | ForEach-Object {
        $destPath = $_.FullName.Replace((Get-Location).Path + "\MATERIALES", (Get-Location).Path)
        if ($_.PSIsContainer) {
            New-Item -ItemType Directory -Path $destPath -Force | Out-Null
        } else {
            Copy-Item $_.FullName -Destination $destPath -Force
...
git rm -rf . --quiet
if (Test-Path "SERVIDOR") {
    Get-ChildItem "SERVIDOR" -Recurse | ForEach-Object {
        $destPath = $_.FullName.Replace((Get-Location).Path + "\SERVIDOR", (Get-Location).Path)
        if ($_.PSIsContainer) {
            New-Item -ItemType Directory -Path $destPath -Force | Out-Null
        } else {
            Copy-Item $_.FullName -Destination $destPath -Force
